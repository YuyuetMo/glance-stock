...
| 价格高亮 | 价格变动时卡片闪烁红 / 绿 |
| 我的持仓 | 同花顺式盈亏视图：每只显示 现价 / 数量 / 成本 / 市值 / 浮动盈亏 / 盈亏率；顶部汇总 总市值 / 总浮动盈亏 / 总收益率——仅记录成本与股数，不交易；支持在命令面板输入**任意代码**实时查询后加为持仓 |
...
| 可拖拽仪表盘 | 首页问候 / 大盘情绪 / 自选·持仓 / 风险 四大模块支持鼠标拖动排序，顺序自动记忆 |
| 自选合并持仓 | 首页「自选 / 持仓」页合并观望股与持仓股，顶部「全部 / 观望 / 持仓」标签切换；持仓卡直接展示盈亏，卡片带「持仓 / 观望 / 港股 / 美股 / 期货」徽章 |
| 多市场检索 | 命令面板 / 添加持仓支持 **A 股 6 位 / 港股 hk00700 / 美股 AAPL / 场内 ETF / 期货·贵金属（沪金、沪银、螺纹、沪铜、铁矿石等）**，实时查询后加入 |
...
| 本地持久化 | 自选股、预警、持仓、账号资料、新闻缓存、首页布局全部存入 IndexedDB |
...
| 文件 | 用途 |
|---|---|
| `Glance-Setup-1.2.0.exe` | 标准安装包，双击安装、可卸载 |
| `Glance-Portable-1.2.0.exe` | 免安装单文件，直接双击运行 |
...
```bash
npm run dist
# 输出：dist-electron/Glance-Setup-1.2.0.exe（安装包）
#       dist-electron/Glance-Portable-1.2.0.exe（免安装单文件）
```
...
- **自选 / 持仓实时行情**：A 股 / 港股 / 美股 / ETF 走腾讯财经接口，期货·贵金属走东方财富接口，断网或接口异常回退本地模拟。

真实持仓盈亏（成本 × 股数 / 手数）基于真实行情价计算。
